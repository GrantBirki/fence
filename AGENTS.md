...
- Full fixed-`ubuntu-24.04` Action acceptance and zero-input drift-canary invocations on `ubuntu-24.04` and `ubuntu-latest` must pass against exact `D` before publication. The immutable `vX.Y.Z` release tag targets `D`, and `action-release.json` maps the version, `M`, `D`, artifact name and digest, manifest schema version, signer workflow, and signer digest.
...
- `script/classify-action-bundle-host`
  ...
  - Validates complete bounded schema-`4` observation evidence and exact Ubuntu 24.04 x64 runner identity, then delegates privileged security verification to the Rust resident.
