...
Host verification is mode-specific. Block modes retain the reviewed privilege and container fingerprint; audit validates the same trusted execution boundary but captures a bounded, complete, stable baseline from the current image instead of requiring historical sudo bytes, runner groups, service states, or local-control topology. Both modes reject unsafe privileged paths, incomplete evidence, unsupported Ubuntu versions, and changes after the resident baseline is established.
...
- `script/classify-action-bundle-host`
  ...
  - Recursively validates the schema-`4` live observation against the bundled binary's schema-`3` `check-support` fingerprint. Block mode compares every enforced executable, permission ancestor, resolver target and mode, sudo marker, group, unit, socket, workload, and local-control field, while delegating the resolver service UID and reviewed README and runner policy digest variations to the Rust resident.
