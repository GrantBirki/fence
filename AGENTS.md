...
This repository is the Rust implementation scaffold for Fence, a security agent intended to harden a narrowly supported CI runner class. The v0 protected target is GitHub-hosted Ubuntu 24.04 x64, selected through either `ubuntu-24.04` or `ubuntu-latest`, with a native Linux GNU agent artifact; see `docs/v0.md` for the normative behavior contract and `docs/threat-model.md` for security claims and residual risks. It is bootstrapped from a hermetic, reproducible, air-gapped-leaning Rust template. Treat it as public open source infrastructure: every file, comment, workflow, log line, and document change may be visible to anyone.
...
- Full fixed-`ubuntu-24.04` Action acceptance and zero-input drift-canary invocations on both `ubuntu-24.04` and `ubuntu-latest` must pass against exact `D` before publication. The immutable `vX.Y.Z` release tag targets `D`, and `action-release.json` maps the version, `M`, `D`, artifact name and digest, manifest schema version, signer workflow, and signer digest.
...
- `script/test-lockdown`
  ...
  - Executes exactly one `acl-reject`, `audit`, `local-control-reject`, `rollback`, `unsafe-preserve`, or `standard` scenario inside a root transient `systemd` service after verifying the selected posture's hosted-runner security contract.
...
- `script/test-selected-profile-runtime`
  ...
  - Bounds each host and Docker TCP DNS listener to four concurrent clients, at most 16 frames per client, and one monotonic two-second deadline per incoming frame; malformed, slow, or oversized client frames do not block other clients, and fatal provenance errors remain visible through the existing five supervised resident workers.
...
- `script/classify-action-bundle-host`
  ...
  - Recursively validates the schema-`4` live observation against the bundled binary's schema-`3` `check-support` fingerprint. Block mode compares every enforced executable, permission ancestor, resolver target and mode, sudo marker, group, unit, socket, workload, and local-control field, while delegating the resolver service UID, reviewed README and runner policy digest variations, and only exact repairable runner-owned `/etc` or `/usr` shapes to the Rust resident.
  - Audit mode validates bounded observation shape and the exact supported operating system, architecture, and runner principal, then delegates all security-critical host decisions to the Rust resident. `FENCE_HOST_MODE=audit` is permitted only for audit acceptance cases; omitted mode remains strict block.
  - Release validation and both runner-label canary legs require normal block-mode activation; transition skips, malformed evidence, unsupported host identity, and block-mode fingerprint drift fail the job.
...
- The non-required `nightly` workflow should run daily and by input-free manual dispatch, fail closed unless the exact selected ref is `main`, build an ephemeral production-shaped candidate from that exact `main` source SHA on fixed `ubuntu-24.04`, and reuse the complete unique-case Action-acceptance suite on `ubuntu-latest`. It is not a branch-protection or release gate. A pass covers only the assigned GitHub-hosted Ubuntu 24.04 x64 image; future Ubuntu major versions are not automatically trusted.
- The non-required `action drift canary` must remain read-only, run every six hours, support manual dispatch with an optional full Action SHA, and expose reusable execution with a required full Action SHA. Scheduled and input-free manual runs resolve the newest non-prerelease immutable release and validate its `action-release.json`. All scheduled, manual, and reusable release-validation calls execute their selected full Action commit on both `ubuntu-24.04` and `ubuntu-latest` with `fail-fast: false`, record the validated runner image version, and require strict block-mode classifier acceptance, zero-input activation, and successful lifecycle verification without skips, fallback, cancellation-dependent cleanup, access restoration, or unbounded diagnostics. Canary results cover only assigned images and do not automatically trust unsupported Ubuntu major versions.
...
- The release workflow must wait for all required checks on source commit `M`, build the Linux x64 artifact exactly once, create and verify signed child distribution commit `D`, run complete Action acceptance and strict canaries on both supported runner labels against `D`, attest the final asset set against `M`, then publish one complete immutable release whose tag targets `D`.
