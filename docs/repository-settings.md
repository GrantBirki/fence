...
- Keep `action drift canary` optional and read-only. Every run tests the selected immutable Action in standard block and audit modes on both `ubuntu-24.04` and `ubuntu-latest`. Validate the release mapping and runner security capabilities, and verify that protection is never restored.
