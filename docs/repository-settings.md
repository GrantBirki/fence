...
- Keep `nightly` optional and outside release requirements. It builds the exact `main` commit on `ubuntu-24.04`, then runs the complete Action acceptance suite on `ubuntu-latest`. Both runner labels are supported, but a passing run confirms only the current `ubuntu-latest` image; it does not automatically trust future images or change the fixed release target.
- Keep `action drift canary` optional and read-only. Scheduled runs test the newest immutable stable release on `ubuntu-24.04` and `ubuntu-latest`; explicit-SHA and release-validation runs use `ubuntu-24.04` only. Every run must validate the release mapping, check the runner fingerprint, activate standard block mode, and verify that access is not restored.
