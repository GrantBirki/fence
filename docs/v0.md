...
- the non-quiet zero-input acceptance job verifies all 40 denied direct-IP burst events and their counters within ten seconds, returning early when complete and never resending the burst; timeout diagnostics contain only bounded counters and up to eight missing fixed test destinations. The read-only downstream verifier proves that its single post-hook structured report retains both final blocked UDP destinations while explicitly disclosing omitted rows; the three quiet finalization replicas remain synthetic-traffic-free, require healthy verified controls, and may report only fully classified noncritical DNS refusals without other warnings or omissions;
