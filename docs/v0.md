...
Both runner labels are release-validated. The selected host must still be GitHub-hosted Ubuntu 24.04 x64 and satisfy the selected mode's security contract. A future Ubuntu major version selected by `ubuntu-latest` remains unsupported until explicitly reviewed.
