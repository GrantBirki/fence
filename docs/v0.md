...
Both runner labels are release-validated. Fence accepts only GitHub-hosted Ubuntu 24.04 x64 and verifies the security properties required by the selected mode. Before trusted executable capture, the root resident may restore root ownership only to the exact reviewed `/etc` and `/usr` directories when their existing ownership and access match the known hosted-image shape. A future Ubuntu major version remains unsupported until explicitly reviewed.
...
Fence supports GitHub-hosted Ubuntu 24.04 x64 selected through `ubuntu-24.04` or `ubuntu-latest`. The classifier validates bounded observation shape and runner identity, while the Rust resident verifies trusted executable paths, effective runner access, sudo policy, container controls, and local services before readiness. Routine runner-image changes never bypass these checks. The fixed Azure-agent identity remains observation evidence rather than an activation input.
...
| Required group consequence | Runner groups remain a subset of reviewed groups; supported Docker access is verified before lockdown |
...
| Passwordless sudo classification | Both block modes remove the `runner` sudo drop-in and reject any remaining runner sudo grants |
...
| Container activation paths | Standard block stops and runtime-masks the reviewed Docker and containerd activation paths |
...
- validate the supported runner, trusted executable paths, and bounded root-owned sudo policy before mutation;
- capture the runner sudo drop-in bytes, mode, owner, device, inode, and digest only in memory, then compare that source again immediately before removal;
- fail if any runner sudo grant remains after removing the reviewed drop-in;
...
For protected block mode with `container_policy: "disable"`, Fence must remove ordinary Docker/containerd control paths that would let later workflow code regain host-equivalent authority. It rejects existing workloads, stops and runtime-masks reviewed Docker/containerd activation paths, verifies daemon and socket access is unavailable, and allows only reviewed root-service identities. The actual post-lockdown inventory becomes the resident baseline; added endpoints, owner changes, unstable scans, or incomplete scans are critical. Tests must cover:
...
A separate non-required drift canary runs every six hours, supports manual dispatch with an optional full Action SHA, and requires an explicit full SHA when invoked as a reusable release-validation workflow. Every invocation runs zero-input standard block on both `ubuntu-24.04` and `ubuntu-latest`; neither runner label may skip activation.
