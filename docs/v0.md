...
The floating `ubuntu-latest` label is regularly compatibility-tested. When it resolves to the reviewed GitHub-hosted x64 runner image and passes the same fingerprint, Fence can run the same protected lifecycle. `ubuntu-latest` is not an additional release-validated security target; an unsupported image fails before mutation.
