...
| Image label | `ubuntu-24.04` or `ubuntu-latest`, resolving to Ubuntu 24.04 |
| Architecture | `x86_64` |
...
Both runner labels are release-validated. The selected host must still be GitHub-hosted Ubuntu 24.04 x64 and satisfy the selected mode's security contract. If the hosted image leaves exactly `/etc` or `/usr` owned by the runner and its primary group, mode `0755`, and runner-writable, the root resident may restore only those exact canonical directories to root ownership before trusted executable capture. It must recheck safe ownership and access, reject every other writable trusted ancestor, and never restore the unsafe state. A future Ubuntu major version selected by `ubuntu-latest` remains unsupported until explicitly reviewed.

Except for that narrowly verified root-ownership correction, Fence must inspect required capabilities before changing firewall, sudo, or container state. If the environment cannot satisfy the selected mode's required capabilities, the invocation fails before reporting ready.
...
Fence v0 supports GitHub-hosted Ubuntu 24.04 x64 runners selected through `ubuntu-24.04` or `ubuntu-latest`; it is not a generalized Linux-host mutation strategy. `script/observe-hosted-runner` records bounded host facts before activation. The root resident first repairs only an exact reviewed runner-owned `/etc` or `/usr` shape and verifies both directories are root-owned and inaccessible to runner writes before capturing trusted executable descriptors. Block modes then compare the reviewed security-sensitive shape against production fingerprint schema `3`, while audit mode validates the same trusted execution boundary and pins a bounded, complete, stable baseline from the current runner image. The fixed Azure-agent identity remains observation evidence rather than an activation input.
...
Protected activation requires a stable, within-bounds observation with complete reachability and ownership. Before mutation, block modes require their retained root container, runner-reachable root Unix, and root TCP inventory to match the schema-`3` accepted reference. Standard block may remove accepted container processes or owners while stopping the measured runtimes and may remove one exact fingerprint-tagged Docker Unix listener only after its reviewed `dockerd` owner exits and the accepted `docker.socket` unit stops. No other endpoint, owner, identity, or multiplicity may change. Standard block pins the verified reduction, degraded block pins the exact accepted inventory, and audit pins the complete stable live inventory without requiring it to equal a previous image's topology. Every mode rechecks its own resident baseline before readiness and every five seconds afterward. Instability, incomplete acquisition, unsupported privilege changes, invalid reductions, or later baseline drift fails closed before readiness or becomes a terminal critical finding after readiness.

The mutation probe is limited to `/`, `/etc`, `/etc/sudoers.d`, `/usr`, `/usr/bin`, and `/usr/sbin`; it uses an exclusive empty file, immediately deletes it as the runner when possible, and requires root cleanup after a partial result. It does not modify commands or sudo policy. The observer does not emit sudo policy text, environment values, process arguments, workload identifiers, arbitrary host files, credentials, or payload data. Observation output remains `observation_only_no_protection`. `check-support` remains read-only and does not claim that protection is active. Block-mode `run` fails before security-state mutation unless the host satisfies the reviewed schema-`3` contract, including effective path access, semantically equivalent sudo policy, and the local-control inventory. Audit instead requires safe trusted paths, preserved sudo and Docker, and complete stable live sudo and local-control baselines. The candidate classifier validates the complete schema-`4` envelope: block mode compares all enforced reference fields while deferring reviewed README and runner policy formatting to the Rust resident, and audit mode delegates its mode-specific security decision to that same resident. Release and canary runs never accept classifier skips.

Publishing fingerprint schema `3` does not change observer schema `4`, bundle-manifest schema `4`, runtime-evidence schema `5`, or the Action input contract. Both reviewed runner labels must resolve to GitHub-hosted Ubuntu 24.04 x64. The observer's sudo `sha256` value is interpreted by the accepted source's required digest profile. Only the exact reviewed `drop_in` / `90-cloud-init-users` identity may use `cloud_init_generated_header_v1`; reviewed README and runner source variations must remain semantically equivalent, while all other effective policy remains exact. The generated-header profile rejects missing, malformed, or repeated headers and does not normalize body comments, whitespace, rules, or line endings. Activation pins raw whole-file SHA-256 values so any later byte change remains detectable.
...
| Sudo policy integrity | `sudoers` remains exact; `README` and `runner` accept only reviewed semantically equivalent formatting; `90-cloud-init-users` uses a domain-separated SHA-256 over the exact bytes following one validated cloud-init generated header; raw whole-file digests remain pinned for runtime mutation checks |
...
The selected mode's host contract is enforced by the trusted-launcher lifecycle and proved by destructive hosted integration. Safe image changes may be accepted only when every required security property remains verified; unsafe drift fails closed.
...
Audit mode does not disable sudo and may not claim protection from sudo-based bypass. It validates protected executable paths, effective access, resolver authority, preserved runner sudo and Docker access, and a bounded, complete, stable baseline of the live sudo policy and local-control inventory. Audit may accept safe image changes to group membership, policy contents, service states, and local-service topology, but it rejects unsafe ownership, writable privileged paths, unavailable controls, incomplete observations, and any later drift from the captured baseline.
...
If required container lockdown cannot be verified, protected block mode fails before readiness. Fence v0 does not adaptively mutate unrecognized runtime layouts; a changed privileged capability or unsupported runtime layout requires review.
...
Audit mode preserves containers, verifies that Docker remains available, pins the complete live local-control baseline, and reports observation only.
...
- Packaging and required source checks use fixed Ubuntu 24.04 runners; compatibility and release-canary coverage exercise both supported Ubuntu labels.
...
A separate non-required `nightly` workflow runs daily and by input-free manual dispatch, fails closed unless the exact selected ref is `main`, builds an ephemeral production-shaped candidate from that exact `main` source SHA on fixed `ubuntu-24.04`, and reuses the complete unique-case Action-acceptance suite on `ubuntu-latest`. It is not a branch-protection or release gate. A green result covers only the Ubuntu 24.04 image assigned to that job; future Ubuntu major versions are not automatically trusted.

A separate non-required drift canary runs every six hours, supports manual dispatch with an optional full Action SHA, and requires an explicit full SHA when invoked as a reusable release-validation workflow. Scheduled and input-free manual runs select the newest non-prerelease immutable release only after validating its `action-release.json` mapping. Every scheduled, manual, and release-validation call runs the selected full Action commit on both `ubuntu-24.04` and `ubuntu-latest` with `fail-fast: false`, records the validated runner image version, and requires normal strict block-mode classification plus zero-input standard activation. Canary results describe only the images assigned to those jobs and do not automatically trust future image builds or Ubuntu major versions.
