...
Fence supports GitHub-hosted Ubuntu LTS x64 runners selected through `ubuntu-24.04` or `ubuntu-latest`. Approved versions are even-year `.04` releases from 24.04 through 40.04. The classifier validates bounded observation shape and runner identity, while the Rust resident verifies trusted executable paths, effective runner access, sudo policy, container controls, and local services before readiness. Routine image updates never bypass these checks. Azure platform-agent correlation is optional diagnostic evidence and never an activation requirement.
