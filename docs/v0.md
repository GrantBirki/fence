...
`ubuntu-latest` is also supported as a floating GitHub-hosted x64 runner label when its current image passes the same reviewed fingerprint. Scheduled tests check it regularly, but the fixed release-validation and protected security target remain `ubuntu-24.04`; an unsupported image fails before mutation.
